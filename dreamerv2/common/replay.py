...
class Replay:
    ...
    def __init__(
      ...
      prioritize_ends=False, free_ram_capacity=-1, sym=0,):
    ...
    def add_step(self, transition, worker=0):
        ...
        if transition['is_last']:
            ...
            if self._sym:
              for k in range(self._sym):
                  _eps = deepcopy(episode)
                  _eps['image'] = []
                  _eps['action'] = []
                  for j in range(len(episode["image"])):
                      theta, trans, pivot = get_random_transform_params(episode['image'][0].shape)
                      ...
                      _eps["action"].append(action_new)
                  self.add_episode(_eps)
            episode.clear()
